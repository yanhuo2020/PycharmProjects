...
def innerL(i, oS):
    ...
    if ((oS.labelMat[i] * Ei < -oS.tol) and (oS.alphas[i] < oS.C)) or (
        ...
        oS.alphas[i] += oS.labelMat[j] * oS.labelMat[i] * (
                    alphaJold - oS.alphas[j])  # 根据公式a1^new = a1^old+y1*y2*(a2^old-a2^new)
...
def draw_points(dataArr, classlabel, w, b, alphas):
    ...
    ax.set_ylabel("y")
    ax1 = ax.scatter(red_points_x, red_points_y, s=30, c='red',
                     marker='s')  # s是shape大小，c是颜色，marker是形状，'s'代表是正方形,默认'o'是圆圈
    ...
    plt.legend([ax1, ax2, ax3, ax4], ["red points", "blue points", "decision boundary", "support vector"],
               loc='lower right')  # 标注
    plt.show()

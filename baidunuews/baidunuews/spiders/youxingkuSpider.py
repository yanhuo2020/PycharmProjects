...
class YouxingkuspiderSpider(scrapy.Spider):
    name = 'youxingkuSpider'
    allowed_domains = ['youxingku.com']
    start_urls = ['https://www.youxingku.com/']
    def __init__(self):
    ...
    def parse(self, response):
        ...
        while pageNumber < 300 :
            ...
            data = {"pageSize": 9,"sortvar":1,"types":3,"page":pageNumber}
            try:
                response = session.post('https://www.youxingku.com/starlibrary/channelpage/listChannel', headers=headers, data=data, timeout=10)
                ...
                continue
            response.close()
            start_resp = response.content  # 响应文件中有粉丝和关注
            try:
                star_resp = json.loads(start_resp)
            except JSONDecodeError:
                print("Error:明星信息抓取失败")
            else:
                if "items" in star_resp:
                    for items in star_resp['items']:
                        '''time.sleep(2)'''
                        starInfo = items['fields']
                        id = starInfo['id']
                        # 使用execute()方法执行SQL语句
                        self.cursor.execute("SELECT * FROM starinfo where url = %s", ('https://www.youxingku.com/starlibrary/Stardetail/index/id/'+format(id)+'/p_type/3.html'))
                        # 使用fetall()获取数据
                        data = self.cursor.fetchone()
                        if data is not None:
                            continue
                        star = {}
                        star['name'] =starInfo['names']
                        star['sex'] = ''
                        star['vocation'] = ''
                        star['url'] = 'https://www.youxingku.com/starlibrary/Stardetail/index/id/'+format(id)+'/p_type/3.html'
                        print(star)
                        self.processStar(star)
                        pageNumber1 = 1
                        while True :
                            data = {"page": pageNumber1, "starid": id}
                            try:
                                response = session.post('https://www.youxingku.com/starlibrary/stardetail/ajaxCommerceList',headers=headers, data=data, timeout=10)
                            except requests.exceptions.RequestException as e:
                                print(e)
                                self.process_request(True)
                                session.proxies = self.current_proxy.proxy
                                continue
                            response.close()
                            daiyan_resp = response.content  # 响应文件中有粉丝和关注
                            try:
                                daiyan_resp = json.loads(daiyan_resp)
                            except JSONDecodeError:
                                print("Error:明星信息抓取失败")
                                continue
                            else:
                                number = 0
                                for items in daiyan_resp:
                                    bname= items['bname']
                                    brand = items['brand']
                                    prod = items['prod']
                                    info = {}
                                    info['name'] = star['name']
                                    info['bname'] = bname.replace(' ', '').replace('\n', '')
                                    info['brand'] = brand.replace(' ', '').replace('\n', '')
                                    info['prod'] = prod
                                    print(info)
    ...
    def processSpeak(self,speak):
        # 使用execute()方法执行SQL语句
        self.cursor.execute("SELECT * FROM starspeakinfo where name = %s and brand= %s",(speak['name'],speak['brand']))
        # 使用fetall()获取数据
        data = self.cursor.fetchone()
        if data is None:
            self.cursor.execute(
                "insert into starspeakinfo(name,bname,prod,brand) values (%s,%s,%s,%s)",
                (speak['name'],speak['bname'],speak['prod'],speak['brand']))
            self.db.commit()  # 提交数据
        else:
            pass
